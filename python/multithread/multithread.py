...
def background_task(name, max_steps):
    ...
    steps = random.randint(
        1, max_steps
    )  # Each task will have a random number of steps up to the user-provided max
    for i in range(steps):
        ...
        if log_type == "INFO":
            ...
            print(
                f"[DEBUG] Task {name}: Processed data chunk {random.randint(1, 100)}."
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run multiple background tasks with randomized logs."
    )
    parser.add_argument("num_threads", type=int, help="Number of threads to run")
    parser.add_argument(
        "max_steps", type=int, help="Maximum number of steps each thread can have"
    )
