...
def analyse_pipeline(pipeline_id:str, threshold:int, cadence:int)->list:
    ...
    current_page = 1
    while True:
        url = f"{base_url}?status=RUNNING&page={current_page}&pipeline_ids={pipeline_id}"
        response = requests.request("GET", url, headers=headers)
    ...
    print("The average time for the pipeline to run is: " + str(average_time) + " seconds")
    output = []
    if len(running_results) > 0:
        for run in running_results:
            time_running = (now - datetime.fromisoformat(run['createdAt'])).total_seconds()
            print(f"The pipeline has been running for {time_running} seconds")
            if time_running > average_time*threshold:
                print(f"{run.get('pipelineName')} is taking significantly longer than average to run and may be experiencing an issue.")
                output.append({
    ...
    recently_completed = [
        run for run in completed_results
        if run.get('completedAt')
        and (now - datetime.fromisoformat(run['completedAt'])).total_seconds() <= cadence * 60
    ]
    if len(recently_completed) > 0:
        for run in recently_completed:
            time_running = (datetime.fromisoformat(run['completedAt']) - datetime.fromisoformat(run['createdAt'])).total_seconds()
            print(f"The pipeline completed in {time_running} seconds")
            if time_running < average_time*0.5:
                print(f"{run.get('pipelineName')} run at {run['createdAt']} was significantly shorter than average to run and may be experiencing an issue.")
                output.append({
                    'pipeline_name': run.get('pipelineName')  or pipeline_id,
                    'run_id': run.get('id'),
                    'status': run.get('runStatus'),
...
if environment == "PROD":
    ...
    orchestra.set_output(name='result', value=format_pipeline_runs_as_slack_blocks(outputs))
