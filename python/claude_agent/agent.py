...
async def main(
    prompt: str, tools: list[str] = ["Read", "Edit", "Glob"], github_repo: str = None
):
    if github_repo:
...
if __name__ == "__main__":
    ...
    tools = [
        tool.strip()
        for tool in os.getenv("TOOLS", "Read,Edit,Glob,Bash").split(",")
        if tool.strip()
    ]

    asyncio.run(main(prompt=prompt, tools=tools, github_repo=github_repo))
    LIGHTDASH_CHART_IDS = os.getenv(
        "LIGHTDASH_CHART_IDS", "No charts found or Lightdash API key not set."
    )
