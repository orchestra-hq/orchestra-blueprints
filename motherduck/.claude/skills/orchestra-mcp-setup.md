...
When the Orchestra MCP server is not connected, use this reference to set it up before proceeding with the fix workflow.
...
> 2. Add the MCP config to `~/.claude/settings.json` (see `references/mcp-setup.md` for the exact block)
